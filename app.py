...
# 鍐欏叆鑺傛祦锛氱疮绉疦娆″彉鏇村悗鎵嶇湡姝ｆ帹閫佸埌GitHub
_pending_saves = 0
_SAVE_THRESHOLD = 5  # 姣?娆¤闂墠鎺ㄩ€佷竴娆?_last_save_time = 0.0
_SAVE_INTERVAL = 60  # 鑷冲皯闂撮殧60绉掓帹閫?
...
def _save_visits(visits: list, force: bool = False):
    ...
    _pending_saves += 1
    now = time.time()
    # 鑺傛祦锛氱疮绉疭AVE_THRESHOLD娆?鎴?璺濅笂娆℃帹閫佽秴SAVE_INTERVAL绉?鎴?寮哄埗淇濆瓨
    should_save = force or _pending_saves >= _SAVE_THRESHOLD or (now - _last_save_time >= _SAVE_INTERVAL and _pending_saves > 0)
    if should_save:
        _pending_saves = 0
        _last_save_time = now
